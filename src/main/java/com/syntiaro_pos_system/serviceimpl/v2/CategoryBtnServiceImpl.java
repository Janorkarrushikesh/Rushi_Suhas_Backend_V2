...
public class CategoryBtnServiceImpl implements CategoryBtnService {
    ...
    public ResponseEntity<ApiResponse> getButtonById(Long id) {
        try {
            Optional<CategoryButton> buttonlist = categoryBtnRepository.findById(id);
            if (buttonlist.isPresent()) {
                return ResponseEntity.ok(new ApiResponse(buttonlist, true, 200));
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(new ApiResponse(buttonlist, false,"Id Not Found !",404));
            }
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(null, false, "...",500));
    ...
    public ResponseEntity<ApiResponse> deleteById(Long id) {
        try{
            Optional<CategoryButton> data= categoryBtnRepository.findById(id);
            if(data.isPresent())
            {
