...
public class InventoryServiceImpl implements InventoryService {
    ...
    public ResponseEntity<ApiResponse> getStoreId(String storeId , Integer page , Integer size) {
        ...
        return ResponseEntity.ok().body(new ApiResponse(inventoryData,true,200));
    }
        catch (Exception e ) {
    ...
    @Override
    public ApiResponse getInventoryById(Integer serialNo) {
        Optional<Inventory> inventoryData = inventoryRepository.findById(serialNo);
        if (inventoryData.isPresent()) {
            return new ApiResponse(inventoryData.get(), true, "Inventory found", HttpStatus.OK.value());
        } else {
            return new ApiResponse(null, false, "Inventory not found", HttpStatus.NOT_FOUND.value());
        }
    }
    ...
    public ResponseEntity<ApiResponse> deleteById(Integer SerialNo) {
        try{
            Optional<Inventory> data= inventoryRepository.findById(SerialNo);
            if(data.isPresent())
            {
