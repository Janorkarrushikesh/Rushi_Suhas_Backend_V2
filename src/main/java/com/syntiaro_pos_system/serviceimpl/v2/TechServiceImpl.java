        ...
        try{
            ...
            if (hasExceededSessionLimit(username)) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(new com.syntiaro_pos_system.entity.v2.ApiResponse(null, false, "Too many active sessions for this user.", 401));
            }
...
//    ******************************Tech Sign Up ******************************************
    ...
    public ResponseEntity<ApiResponse> registerTech(TechSignupRequest techsignupRequest) {
        ...
        try{
            if (storeRepositry.existsByUsername(techsignupRequest.getUsername())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse(null,false,"Error: Username is already taken!",400));
            }

            if (userRepository.existsByUsername(techsignupRequest.getUsername())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse(null,false,"Error: Username is already taken!",400));
            }

            if (techRepository.existsByUsername(techsignupRequest.getUsername())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse(null,false,"Error: Username is already taken!",400));
            }

            if (superAdminRepository.existsByUsername(techsignupRequest.getUsername())) {
            ...
            if (techRepository.existsByEmail(techsignupRequest.getEmail())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse(null,false,"Error: Email is already in use!",400));

            }

            if (userRepository.existsByEmail(techsignupRequest.getEmail())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse(null,false,"Error: Email is already in use!",400));

            }

            if (storeRepositry.existsByEmail(techsignupRequest.getEmail())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse(null,false,"Error: Email is already in use!",400));

            }

            if (superAdminRepository.existsByEmail(techsignupRequest.getEmail())) {
            ...
            if (techsignupRequest.getPassword().equals(techsignupRequest.getComfirmpassword())) {
                ...
                Set<String> strRoles = techsignupRequest.getRole();
    ...
    public ResponseEntity<ApiResponse> forgotPassword(TechSignupRequest techSignupRequest) {
        try{
            ...
            Optional<Tech> data= techRepository.findByEmail(email);
            if (data.isEmpty()){
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
    ...
    public ResponseEntity<ApiResponse> resetPasseord(Map<String, String> resetRequest) {
        try{
            ...
            //update In Database
            Optional<Tech> optionalTech = techRepository.findByEmail(email);
