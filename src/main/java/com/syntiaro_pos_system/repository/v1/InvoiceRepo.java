...
public interface InvoiceRepo extends JpaRepository<Invoice, Integer >{
    ...
    @Query("SELECT MAX(b.invoiceId) FROM Invoice b WHERE b.storeId = :store_id")
    Integer findLastNumberForStore(@Param("store_id") Integer store_id);


    @Query("SELECT i FROM Invoice i WHERE i.storeId = :store_id AND i.createDate BETWEEN :startDate AND :endDate")
